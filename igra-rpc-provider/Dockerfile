...
# Configure SSH with pinned GitHub host keys.
# Required for fetching private dependency (ssh://git@github.com/IgraLabs/kaswallet.git).
# Without known_hosts, SSH would prompt for host verification and hang the build.
# These are GitHub's official public host keys - pinned for security (prevents MITM attacks).
